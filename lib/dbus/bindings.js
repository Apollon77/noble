...
class DbusBindings extends EventEmitter {
  ...
  async _connect (id) {
    ...
    // Await Connect() and waitConnected together via Promise.all so a
    // reject from _onDeviceDisconnected during the in-flight Connect() call
    // already has a handler attached — otherwise it surfaces as an unhandled
    // rejection and crashes the process (#91).
    try {
